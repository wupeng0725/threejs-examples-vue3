...
// const pointLightCode = await getComponentCode('./components/PointLight.vue');

export const getComponentCode = async (componentPath) => {
  try {
    // 1. 使用 glob 模式匹配所有子目录的 .vue 文件
    const componentCodes = import.meta.glob('/src/components/*.vue', {
      query: '?raw', // 关键参数：以字符串形式加载源码
      eager: false // 默认异步加载
    })
    // 2. 统一路径格式（确保使用绝对路径或统一相对路径）
    const normalizedPath = `/src/components/${componentPath}.vue`
